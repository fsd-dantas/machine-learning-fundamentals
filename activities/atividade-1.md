...
> Generated by [`atividade-1.py`](atividade-1.py) — `SEED = 42`, stratified (Part A) / shuffled (Part B) 5-fold CV, scale-sensitive learners wrapped in a fold-local `StandardScaler` to prevent leakage. All values are means over the five folds; **per-fold values and standard deviations** are printed by the script's `Respostas` section — fold-to-fold σ for the top classifiers (≈0.013–0.018) is the same order of magnitude as the gaps between them, which is why the statistical tests below matter. Re-run with `python atividade-1.py` to reproduce exactly.
...
For **both** models the error rate on the **malignant** class (≈4.3–4.7%) is higher than on the benign class (≈0.8–1.1%): the **malignant class is misclassified more often**. In the abstract framing of the task this is the costlier direction — a missed malignancy (false negative) delays further investigation — though false positives are not free either: the confirmatory follow-up (biopsy) is invasive, with real clinical and psychological burden. *If* such a model informed screening, raising recall on the malignant class at the expense of precision would be the natural threshold adjustment; but actual costs depend on the care pathway, and no deployment conclusion can be drawn from a small, curated, single-source dataset without external validation.
