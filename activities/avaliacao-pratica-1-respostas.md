...
As duas formas de evidência são complementares, e nenhuma substitui a outra. O **teste exato de McNemar** controla o erro amostral do conjunto de teste: como todos os modelos são avaliados sobre as **mesmas** 10.000 imagens, seus erros são pareados por construção, e o teste determina se a diferença observada entre **aqueles dois modelos treinados** excede o que o acaso amostral produziria. Ele **não** informa, contudo, se a mesma ordenação se manteria sob outra inicialização — questão que só a repetição responde. Onde há repetição, ela é reportada; onde não há, a limitação é declarada, e as conclusões sobre ordenação devem ser lidas como **condicionais aos modelos treinados**, e não como propriedades estáveis dos métodos.

A *seed* altera a inicialização dos pesos e a amostragem do aumento de dados, jamais os dados: a subamostra é fixada por um gerador independente.

**Variabilidade entre inicializações.** Onze configurações deste experimento dispõem de duas ou mais execuções, o que permite estimar a sensibilidade dos resultados à inicialização dos pesos e à amostragem do aumento de dados — os dois componentes que a *seed* controla. Excluído o SGD (tratado adiante), o desvio-padrão observado entre *seeds* varia de **0,04 a 0,63 ponto percentual**, com **mediana de 0,20 pp** e amplitude típica inferior a 0,5 pp. O SGD constitui exceção acentuada, com desvio de 3,80 pp.

Esse valor estabelece um **piso de resolução prático** para as comparações deste relatório: diferenças da ordem de 0,2 a 0,4 ponto percentual entre configurações não podem ser distinguidas do efeito da inicialização, e é por essa razão que os vereditos são emitidos por teste estatístico, e não por leitura direta das médias.

**Estabilidade do modelo vencedor.** As três execuções do ViT produziram 0,9825, 0,9815 e 0,9813 — **desvio-padrão de 0,0006**, ou 0,06 ponto percentual, a menor dispersão registrada no experimento. É cerca de **três vezes inferior** à dispersão típica das redes convolucionais (0,20 pp) e **sessenta e três vezes inferior** à do SGD. O ajuste fino de um *transformer* pré-treinado, neste regime, mostra-se notavelmente insensível à inicialização — observação coerente com a hipótese de que o pré-treinamento, e não a otimização, responde pela maior parte de seu desempenho.
...
**A extração de características é, por larga margem, a estratégia mais eficiente:** entrega **14,45 pontos percentuais por minuto** de GPU acima da CNN treinada do zero, contra 3,20 do ViT, 2,86 do ajuste fino e 1,18 do ajuste fino com aumento de dados. Uma ordem de grandeza separa a primeira colocada da segunda nesse critério.

Isso **não contradiz** a tabela de acurácia — o ViT permanece o modelo mais acurado, e por larga margem. O que o eixo de custo estabelece é que as duas perguntas têm **respostas diferentes**: se o critério é acurácia máxima, a resposta é o ViT; se é acurácia por unidade de recurso, a resposta é a extração de características. Um relatório que reportasse apenas a primeira coluna omitiria do leitor o fato de que **87% do ganho de transferência sobre a CNN do zero alcançado pela melhor estratégia convolucional (8,86 dos 10,14 pontos percentuais) é capturado em 36 segundos de GPU**, sem treinar um único parâmetro convolucional.
...
**Resposta: não. Nenhum otimizador supera o Adam de modo estatisticamente significativo.** O AdamW obtém a maior média (0,8660 contra 0,8652 do Adam), mas a diferença de **0,08 ponto percentual** é inferior à metade da dispersão típica entre inicializações medida neste experimento (0,20 pp), e o teste exato de McNemar não a distingue de zero (p = 0,087). O RMSprop fica 0,14 pp abaixo do Adam. **A configuração padrão do enunciado já é adequada**, e a substituição do otimizador não constitui via de melhoria neste delineamento.
...
O SGD produziu **0,8617** em uma *seed* e **0,8080** na outra — **5,4 pontos percentuais de diferença entre execuções que variam apenas na inicialização dos pesos**. Sua dispersão é vinte e nove vezes a do AdamW e mais de vinte vezes a dispersão típica observada nas demais configurações deste experimento (0,20 pp).

Cabe delimitar o que essa evidência sustenta. Com **apenas duas execuções**, não é possível estimar a distribuição dos resultados do SGD nem atribuir probabilidade a desfechos futuros; a afirmação defensável é que **observou-se instabilidade acentuada nesta amostra**, discrepante de todas as demais configurações. A implicação prática, ainda assim, é imediata: **uma execução única do SGD não é base suficiente para avaliá-lo**, ao passo que uma execução única do AdamW ou do Adam o seria — e essa assimetria só se tornou visível porque houve repetição.
...
**Registre-se o que não foi verificado.** Sabe-se que o modelo da fase 1 apresentava **acurácia de validação superior**; não se sabe se ele também superaria o da fase 2 **no conjunto de teste**, pois o *checkpoint* da fase 1 não foi avaliado sobre o teste. A direção do viés é, portanto, **plausível mas não demonstrada**, e afirmá-la exigiria uma execução adicional que este delineamento não contempla.
